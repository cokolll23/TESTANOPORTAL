                ...
                if (this.params.USER_CONSENT === 'Y' && BX.UserConsent) {
                    if (this.options.userConsents) {
                        ...
                        if (requiredUncheckedConsentId) {
                            ...
                            // todo условие сравнения суммы заказа и баллов из 19 инфоблока
                            this.doSaveAction();
                        }
                    } else {
...
// делаем проверку и после у USER баллы и сумма заказа и после этого  нее doSaveAction
        checkBonus: function () {
            console.log(this.result);
            var ORDER_TOTAL_PRICE = this.result.TOTAL.ORDER_TOTAL_PRICE;
            var bonusVal = this.sendRequestBonusVal();

            var bonusDiffVal;

            /* // this.result.TOTAL.PRICE_WITHOUT_DISCOUNT_VALUE цена заказа
             // this.result.TOTAL.ORDER_TOTAL_PRICE цена заказа
             // this.result.IS_ADDRESS_FROM емаил покупателя заказа
             // todo  this.result.properties цикл и если CODE=="EMAIL" его VALUE
             //  емаил покупателя заказа
             // получить значение знач. свойства COLUMN33 элемента выбранного по символьному коду abdr@ya.ru
             // request ajax*/

            if (parseInt(this.result.TOTAL.ORDER_TOTAL_PRICE )<= parseInt(bonusVal)) {
                alert(bonusDiffVal);
                bonusDiffVal = 1;
            }
            if (parseInt(this.result.TOTAL.ORDER_TOTAL_PRICE) >= parseInt(bonusVal)) {
        ...
        sendRequestBonusVal: function () {
            var bonusVal = '';
            BX.ajax({
                url: '/local/ajax/ajax.php',
                data: {
                    email: 'abdr@ya.ru',//this.getUserEmail(),
                    action: 'getBonusVal'
                },
                method: 'POST',
                dataType: 'json',
        ...
        getUserEmail: function () {
            ...
            for (let i = 0; i <= props.length; i++) {
                if (props[i].CODE == "EMAIL") {
                    console.log(props[i].VALUE[0]);
                    emailUserVal = props[i].VALUE[0]
                    break
                }
            }
            return emailUserVal;
        ...
        doSaveAction: function () {
            if (this.isOrderSaveAllowed()) {
                ...
                // todo условие сравнения суммы заказа и баллов из 19 инфоблока
                alert(this.sendRequestBonusVal());
        ...
        editSection: function (section) {
            ...
            var active = section.id == this.activeSectionId,//var active = true Открыть все блоки и убрать лишнее
                titleNode = section.querySelector('.bx-soa-section-title-container'),
        ...
        initUserConsent: function () {
            ...
            BX.ready(BX.delegate(function () {
                if (this.options.userConsents) {
                    if (BX.UserConsent) {
                        var controls = BX.UАserConsent.getItems();
