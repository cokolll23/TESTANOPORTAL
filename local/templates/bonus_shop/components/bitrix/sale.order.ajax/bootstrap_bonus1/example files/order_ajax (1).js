    ...
    BX.Sale && BX.Sale.Input && BX.Sale.Input.Utils && (BX.Sale.Input.Utils.asMultiple = function (value) {
        ...
        initializePrimaryFields: function () {
            ...
            if (result.error) this.showError(this.mainErrorsNode, result.error), this.animateScrollTo(this.mainErrorsNode, 800, 20); else if (result.order.SHOW_AUTH) {
                ...
                this.result = result.order, this.prepareLocations(result.locations), this.locationsInitialized = !1, this.maxWaitTimeExpired = !1, this.pickUpMapFocused = !1, this.deliveryLocationInfo = {}, this.initialized = {}, this.couponsCustomInitialized = !1, this.initOptions(), this.editOrder(), this.mapsReady && this.initMaps(),
            ...
            return maxSize = parseInt(maxSize), len = parseInt(len), maxSize > gbDivider ? parseFloat(maxSize / gbDivider).toFixed(len) + " Gb" : maxSize > 1048576 ? parseFloat(maxSize / 1048576).toFixed(len) + " Mb" : maxSize > 1024 ? parseFloat(maxSize / 1024).toFixed(len) + " Kb" : maxSize + " B"
            ...
            errorContainer && msg.length && (BX.cleanNode(errorContainer),
                errorContainer.appendChild(BX.create("DIV", {html: msg})),
                (animate = !this.hasErrorSection[node.id]) ? (errorContainer.style.opacity = 0, errorContainer.style.display = "", new BX.easing({
                    duration: 300,
                    start: {opacity: 0},
                    finish: {opacity: 100},
                    transition: BX.easing.makeEaseOut(BX.easing.transitions.quad),
            ...
            document.querySelector("div[class*=" + item + "]") && rangeRandom(30, 50) == rangeRandom(30, 50) && (BX.unbindAll(this.totalInfoBlockNode.querySelector("a.btn-order-save")), BX.unbindAll(this.mobileTotalBlockNode.querySelector("a.btn-order-save")), BX.unbindAll(this.orderSaveBlockNode.querySelector("a")),
            ...
            for (i in this.locationsInitialized = !0, this.fixLocationsStyle(this.deliveryBlockId, this.regionHiddenBlockNode), this.fixLocationsStyle(this.propsBlockNode, this.propsHiddenBlockNode), this.locations) {
                ...
                value ? (city_destination.classList.remove("hidden"), this.deliveryBlockNode.querySelectorAll(".destination_value").forEach((function (node) {
            ...
            var date_fld = $('#date_fld').val();
            $('#soa-property-41').val(date_fld);
            /*var isDelayDate=true;
            if (!date_fld){
                isDelayDate=false;
                $('#date_fld').toggleClass('has-errors');
                $('#asx-delay-date').addClass('error-border');
            }else{
                isDelayDate=true;
            }*/
            var formInputSNG = $(".as-pickup-sng").find('input'),
                formInputRUS = $(".as-pickup-select").find('input'),
                arAddressSng, strAddressRus, currDeliveryBlock;

            for (var key in this.result.DELIVERY) {
                if (this.result.DELIVERY[key]['CHECKED'] == 'Y') {
                    currDeliveryBlock = this.result.DELIVERY[key]['ID'];
                }
            }
            if (currDeliveryBlock == 7) {
                var arAddressSng = [], country1, region1,
                    city1, street1, house1,
                    appartment1, description1, countRequered1 = [],
                    strResultAddress1;

                $.each(formInputSNG, function (i, el) {
                    if ($(this).data('required') == 1) {
                        if (Array.isArray(arAddressSng)) {
                            countRequered1.push(1);
                        }

                        if (Array.isArray(arAddressSng)) {
                            if (el.value == '') {
                                $(el).toggleClass('has-errors');
                                $('#bx-soa-delivery').toggleClass('has-errors');
                            } else {

                                arAddressSng.push(el);
                            }
                ...
                if (arAddressSng.length == countRequered1.length) {
                    if ($('#appartment1').val() !== '') {
                        appartment1 = ' , кв. ' + $('#appartment1').val();
                    } else {
                        appartment1 = '';
                    }

                    if($('#unrestricted_value1').val() != ''){
                        strResultAddress1 = $('#unrestricted_value1').val() + appartment1;
                    }

                    //strResultAddress1 = arAddressSng[0].value + ' , ' + arAddressSng[1].value + ' , ' + arAddressSng[2].value + ' , ' + arAddressSng[3].value + appartment1
                    if ($('textarea#as-orderDescription1').val() != '') {
                        description = $('#as-orderDescription1').val();
                    }
                    $('textarea#orderDescription').val(description);
                    $('textarea#soa-property-7').val('').val(strResultAddress1);
                }
            }

            if (currDeliveryBlock != 7) {

                var arAddressRus = [], region,
                    city, street, house,
                    appartment, description, countRequered = [],
                    strResultAddress;

                $.each(formInputRUS, function (i, el) {
                    if ($(this).data('required') == 1) {
                        if (Array.isArray(arAddressRus)) {
                            countRequered.push(1);
                        }
                ...
                if (arAddressRus.length == countRequered.length) {
                    if ($('#appartment').val() != '') {
                        appartment = ' , кв. ' + $('#appartment').val();
                    } else {
                        appartment = '';
                    }
                    if($('#unrestricted_value').val() != ''){
                        strResultAddress = $('#unrestricted_value').val() + appartment;
                    }

                    //strResultAddress = arAddressRus[0].value + ' , ' + arAddressRus[1].value + ' , ' + arAddressRus[2].value + ' , ' + arAddressRus[3].value + appartment

                    if ($('textarea#as-orderDescription').val() != '') {
                        description = $('#as-orderDescription').val();
                    }
                    $('textarea#orderDescription').val(description);
                    //alert(strResultAddress);
                    $('textarea#soa-property-7').val('').val(strResultAddress);
                }
            }


            return this.isValidForm() && (this.allowOrderSave(),
                "Y" === this.params.USER_CONSENT && BX.UserConsent ? BX.onCustomEvent("bx-soa-order-save", []) : this.doSaveAction()), BX.PreventDefault(event)
        },
        doSaveAction: function () {
            ...
            showNode && !BX.hasClass(showNode, "bx-selected") && (fadeNode && "bx-soa-auth" !== this.activeSectionId && this.fade(fadeNode), this.show(showNode))
            ...
            if (this.orderBlockNode && this.result) {
                BX.addClass(this.deliveryBlockNode, "bx-active"), this.deliveryBlockNode.removeAttribute("style"), this.mobileTotalBlockNode.style.display = this.result.SHOW_AUTH ? "none" : "";
            ...
            if (this.result.PERSON_TYPE) {
                ...
                if (personTypesCount > 5) {
                    ...
                    for (i in this.result.PERSON_TYPE) this.result.PERSON_TYPE.hasOwnProperty(i) && (currentType = this.result.PERSON_TYPE[i], label = BX.create("LABEL", {
                        ...
                        }), BX.util.htmlspecialchars(currentType.NAME)], events: {
                    ...
                    if (this.regionBlockNotEmpty = !0, personTypesCount > 2) {
                        ...
                        node.appendChild(BX.create("SELECT", {
                            ...
                            }, children: options, events: {change: BX.proxy(this.sendRequest, this)}
                        })), this.regionBlockNotEmpty = !0
                    }
            ...
            if (this.result.PAY_SYSTEM && (0 != this.result.PAY_SYSTEM.length || "Y" == this.result.PAY_FROM_ACCOUNT)) {
                ...
                if (currentPaySystem && (paySystemNode = BX.findChildren(this.paySystemBlockNode, {
            ...
            const childrenBlocks = [],
                $deliveryLabel = BX.create("div", {props: {className: "bx-compact-title"}, html: currentDelivery.NAME});
            ...
            let city_destination = this.deliveryBlockNode.querySelector(".city_destination"),
                change_city = this.deliveryBlockNode.querySelector(".change_city");
            if (BX.Sale.OrderAjaxComponent && Object.entries(BX.Sale.OrderAjaxComponent.locations).length) {
                city_destination.classList.remove("hidden"), change_city.classList.remove("hidden");
            ...
            } else city_destination.classList.add("hidden"), change_city.classList.add("hidden");
        ...
        editDeliveryItems: function (deliveryNode) {
            ...
            if (this.result.DELIVERY && this.result.DELIVERY.length) {
            ...
            var priceNodesArray;
            return void 0 !== delivery.DELIVERY_DISCOUNT_PRICE && parseFloat(delivery.DELIVERY_DISCOUNT_PRICE) != parseFloat(delivery.PRICE) ? parseFloat(delivery.DELIVERY_DISCOUNT_PRICE) > parseFloat(delivery.PRICE) ? [delivery.DELIVERY_DISCOUNT_PRICE_FORMATED] : [delivery.DELIVERY_DISCOUNT_PRICE_FORMATED, BX.create("BR"), BX.create("SPAN", {
                props: {className: "bx-price-old"},
            ...
            item.PERIOD_TEXT =item.PERIOD_TEXT+ ', после того как с вами свяжутся для подтверждения заказа';
            ...
            if (deliveryCheckbox || (deliveryCheckbox = this.deliveryHiddenBlockNode.querySelector("input[type=radio][name=DELIVERY_ID]:checked")),
            deliveryCheckbox || (deliveryCheckbox = this.deliveryHiddenBlockNode.querySelector("input[type=hidden][name=DELIVERY_ID]")), deliveryCheckbox) for (i in deliveryId = deliveryCheckbox.value, this.result.DELIVERY) if (this.result.DELIVERY[i].ID == deliveryId) {
        ...
        saveProfile: function () {
            ...
            if (!propsErrors.length) if (this.propsBlockNode.querySelector(".alert.alert-danger").style.display = "none", BX.cleanNode(this.propsBlockNode.querySelector(".alert.alert-danger")), BX.removeClass(this.propsBlockNode, "bx-step-error"), this.hasUserProfiles()) {
            ...
            if (this.hasUserProfiles() && "Y" === this.params.ALLOW_USER_PROFILES) {
        ...
        isValidRegionBlock: function () {
            ...
            return regionErrors.concat(this.isValidPropertiesBlock(!1, this.deliveryBlockNode))
        },
        getValidationDataPhone: function (arProperty, propContainer) {
            ...
            return data
        }, getValidationDataPhone1: function (arProperty, propContainer) {
            var data = {}, inputs;

            switch (arProperty.TYPE) {
                case"STRING":
                    if (data.action = "blur", data.func = BX.delegate((function (input, fieldName) {
                        return this.validatePhone1(input, arProperty, fieldName)
                    }), this), inputs = propContainer.querySelectorAll("input.phone"), $(inputs).length) {
                        data.inputs = inputs;
                        break
                    }
        ...
        bindValidationPhone: function (id, propContainer) {
            if (this.validation.properties && this.validation.properties[id]) {
                var arProperty = this.validation.properties[id], i, k;
                var data = this.getValidationDataPhone(arProperty, propContainer);
                if (data && data.inputs && data.action && "object" == typeof appAspro && appAspro && appAspro.phone) for (i = 0; i < $(data.inputs).length; i++) BX.type.isElementNode(data.inputs[i]) && (appAspro.phone.init($(data.inputs[i])), BX.bind(data.inputs[i], data.action, BX.delegate((function () {
                    this.isValidProperty(data)
                }), this)))
            }
        },

        validatePhone: function (input, arProperty, fieldName) {// если не  заполнены адрес доставки телефон фио
            var bl = 0;
            for (var key in this.result.DELIVERY) {
                if (this.result.DELIVERY[key]['CHECKED'] == 'Y') {
                    if (this.result.DELIVERY[key]['ID'] != 36) {
                        bl = 1;
                    }
                    if (this.result.DELIVERY[key]['ID'] == 36) {
                        bl = 0;
            ...
            if (bl == 0) {
                var msgf = BX.message("SOA_FIELD1")
            }
            if (bl == 1) {
                msgf = BX.message("SOA_FIELD2")
            }

            if (!input || !arProperty) return [];
            var value = input.value, errors = [], name = BX.util.htmlspecialchars(arProperty.NAME),
                code = BX.util.htmlspecialchars(arProperty.CODE);

            if (code == 'ADDRESS') {
                var field = msgf, re;
            } else {
                field = BX.message("SOA_FIELD") + ' "' + name + '" ' + BX.message("SOA_REQUIRED"), re;
            }


            return "Y" == arProperty.REQUIRED && 0 == value.length && errors.push(field),
            "Y" == arProperty.IS_PHONE && value.length > 0 && (this.validateNormalPhone(input, errors, field),
                this.validateIntlPhone(input, errors, field)), errors.filter(Boolean)


        }, validatePhone1: function (input, arProperty, fieldName) {// если не  заполнены адрес доставки телефон фио
            ...
            if (code == 'ADDRESS') {
                var field = BX.message("SOA_FIELD") + '-- validatePhone1', re;
            } else {
                field = BX.message("SOA_FIELD") + ' "' + name + '" ' + BX.message("SOA_REQUIRED"), re;
            }
            ...
            if (appAspro.phone.checkIntlPhone) {
                const telInput = $(input).data("iti"), validPhone = telInput.isValidNumber();
            ...
            return "Y" == arProperty.MULTIPLE && "Y" !== arProperty.IS_LOCATION ? errors : ("Y" != arProperty.REQUIRED || 0 != value.length && value != BX.message("SOA_NOT_SPECIFIED") || errors.push(field + " sdggdg " + BX.message("SOA_REQUIRED")), errors)
            ...
            if (this.totalInfoBlockNode && this.result.TOTAL) {
                ...
                if (params = {}, (deliveryError = (curDelivery = this.getSelectedDelivery()) && curDelivery.CALCULATE_ERRORS && curDelivery.CALCULATE_ERRORS.length) ? (deliveryValue = BX.message("SOA_NOT_CALCULATED"), params.error = deliveryError) : 0 === parseFloat(total.DELIVERY_PRICE) ? (deliveryValue = this.params.MESS_PRICE_FREE, params.free = !0) : deliveryValue = total.DELIVERY_PRICE_FORMATED, this.result.DELIVERY.length && this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message("SOA_SUM_DELIVERY"), deliveryValue, params)), this.options.showDiscountPrice && (discText = this.params.MESS_ECONOMY, total.DISCOUNT_PERCENT_FORMATED && parseFloat(total.DISCOUNT_PERCENT_FORMATED) > 0 && (discText += total.DISCOUNT_PERCENT_FORMATED), this.totalInfoBlockNode.appendChild(this.createTotalUnit(discText + ":", total.DISCOUNT_PRICE_FORMATED, {highlighted: !0}))), parseFloat(total.PAY_SYSTEM_PRICE) >= 0 && this.result.DELIVERY.length && this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message("SOA_PAYSYSTEM_PRICE"), "~" + total.PAY_SYSTEM_PRICE_FORMATTED)), !this.result.SHOW_AUTH) {
                    "Y" === this.params.SHOW_COUPONS && this.editCoupons(this.totalInfoBlockNode), this.showTotalDeliveryInfo(), this.showTotalPaymentInfo(), this.totalInfoBlockNode.appendChild(BX.create("DIV", {
            ...
            return name = name || "", value = value || "",
                totalValue = (params = params || {}).error ? [BX.create("A", {
