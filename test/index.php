...
//=======АНО=================================================================
//$arUsersAno = json_decode(file_get_contents('Ano.json'));
...
/*foreach ($arUsersKomitet as $userFields) {
    //IH::addElsToIblock('sotrudniki', $userFields['Фамилия'].' '.$userFields['Имя'],$userFields['E-Mail'] );
    $explUserFields=explode(' ',$userFields['ФИО']);
    try {
        $result = ElementSotrudnikiTable::add([
            'NAME' => $explUserFields[0].' '.$explUserFields[1],
            'CODE' => $userFields['E-mail'],
            'SECTION_ID' => 33,
            'ACTIVE' => 'Y',
        ]);
        ...
        if ($result->isSuccess()) {
            ...
            echo "Элемент успешно добавлен с ID: " . $elementId;
        } else {
