        ...
        <div class="mb-1 basket-line-block">
            <? if ($USER->IsAuthorized()):
                ...
                <a class="basket-line-block-icon-profile"
                   href="<?= $arParams['PATH_TO_PROFILE'] ?>"><?= htmlspecialcharsbx($name) ?></a>
                <a style='margin-right: 0;'
                   href="?logout=yes&<?= bitrix_sessid_get() ?>"><?= GetMessage('TSB1_LOGOUT') ?></a>
            <? else:
            ...
            ?>
                <a class="basket-line-block-icon-profile"
                   href="<?= $pathToAuthorize ?>"><?= GetMessage('TSB1_LOGIN') ?></a>
            <?
    ...
    <div class="basket-line-block">
        ...
        if (!$compositeStub) {
            if ($arParams['SHOW_NUM_PRODUCTS'] == 'Y' && ($arResult['NUM_PRODUCTS'] > 0 || $arParams['SHOW_EMPTY_VALUES'] == 'Y')) {
                ...
                <? if ($USER->IsAuthorized()): ?>
                    <div id="lx-total_quantity" class="l-total_quantity">
                        Количество товаров:  <?= $arResult['TOTAL_QUANTITY']; ?>
                    </div>
                <? endif; ?>
